...
class PincelDeLuzSupabaseUser extends BaseAuthUser {
  PincelDeLuzSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> pincelDeLuzSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PincelDeLuzSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
